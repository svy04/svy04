...
| Board readiness gates | Board v1 collection plan, required evidence atoms, stop conditions, `verify_minecraft_board_v1_gate.py` promotion guard, `manifest-preflight.json`, `MANIFEST-CONTRACT.md`, `manifest.schema.json`, manifest contract/schema, aggregate transcript ledger, `scorer-transcript-availability.json`, `raw-transcript-preflight.json`, local per-arm build/render logs, and a local wrong-anchor execution/render sidecar receipt. | Blocker contract only; board v1 is not ready. `READY.json`, `redacted-screenshots/manifest.json`, route-linked board-v1 entries, comparable wrong-anchor score, raw per-judge score rows, raw comments, redaction-reviewed raw rows, and full per-judge scorer transcript are still missing, so this does not add external validation or stronger proof. |
...
The latest private plugin gate adds a board v1 collection plan, `verify_minecraft_board_v1_gate.py`, one local wrong-anchor execution/render sidecar, local per-arm build/render logs, `MANIFEST-CONTRACT.md`, `manifest.schema.json`, and `raw-transcript-preflight.json`. This is a blocker contract, not stronger proof; the sidecar, manifest contract/schema, and raw transcript preflight are only receipts/contracts. It lists evidence atoms, required route-linked wrong-anchor scoring evidence, scoring transcript fields, raw transcript row fields, public-safe artifact requirements, and stop conditions before any board v1 claim can be made. Board v1 is not ready.
