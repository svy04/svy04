...
| Board readiness gates | Board v1 collection plan, required evidence atoms, stop conditions, `verify_minecraft_board_v1_gate.py` promotion guard, `manifest-preflight.json`, `MANIFEST-CONTRACT.md`, `manifest.schema.json`, manifest contract/schema, aggregate transcript ledger, local per-arm build/render logs, and a local wrong-anchor execution/render sidecar receipt. | Blocker contract only; board v1 is not ready. `READY.json`, `redacted-screenshots/manifest.json`, route-linked board-v1 entries, comparable wrong-anchor score, and full per-judge scorer transcript are still missing, so this does not add external validation or stronger proof. |
